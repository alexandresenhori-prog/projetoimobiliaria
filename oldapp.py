...
app.secret_key = os.getenv('SECRET_KEY', 'xando_projeto_imobiliaria')
...
def listar_perfis():
    ...
    try:
        conn = get_db_connection()
        cur = conn.cursor(cursor_factory=RealDictCursor)
        cur.execute("SELECT * FROM perfis ORDER BY nome ASC")
        perfis = cur.fetchall()
        cur.execute("SELECT * FROM permissoes ORDER BY modulo, acao")
        permissoes_disponiveis = cur.fetchall()
        return render_template('perfis.html', perfis=perfis, permissoes=permissoes_disponiveis)
    except Exception as e:
        return f"Erro ao carregar perfis: {e}"
    finally:
        if cur: cur.close()
        if conn: conn.close()
...
def salvar_perfil():
    ...
    permissoes_selecionadas = request.form.getlist('permissoes') # Pega os IDs dos checkboxes
    
    conn = get_db_connection()
    cur = conn.cursor()
    try:
        # 1. Insere o novo perfil
        cur.execute("INSERT INTO perfis (nome, descricao) VALUES (%s, %s) RETURNING id", (nome, descricao))
        perfil_id = cur.fetchone()[0]
        
        # 2. Vincula as permissões selecionadas ao perfil
        for perm_id in permissoes_selecionadas:
            cur.execute("INSERT INTO perfil_permissoes (perfil_id, permissao_id) VALUES (%s, %s)", (perfil_id, perm_id))
